...
var dx = []; //movement for bug X value
var dy = []; //movement for bug Y value
...
function setup()
{
  ...
  for (var i=0; i<numBug; i++)
  {
    ...
    bugColor.push(color(random(180,240),random(45,240),random(100,240)));
    dx.push(-3,3);
    dy.push(-3,3);
  }
...
function draw()
{
  ...
  for (var i=0; i<numLeafs; i++)
  {
   drawLeafs(leafX[i],leafY[i],leafColor[i],leafRot[i],leafScale[i]);   //issue: Using leafColor[i] removes moving bugs? Why!?!?!
  }
...
function drawLeafs(x,y,colorleaf,rot,sc)
{
  push(); 
...
function drawBug(x,y,colorbug,dx,dy)
{
  push();
